...
# Optional, for logging CPU and RAM usage.  Useful for making appropriate sized
# workers:
# library(autometric)

# You'll need to replace this with your HPC group name.  You can discover the
# name by running `va` when logged into the HPC.
hpc_group <- "kristinariemer"

# Detect whether you're on HPC & not with an Open On Demand session (which
# cannot submit SLURM jobs) and set appropriate controller
slurm_host <- Sys.getenv("SLURM_SUBMIT_HOST")
hpc <- grepl("hpc\\.arizona\\.edu", slurm_host) & !grepl("ood", slurm_host)

# Note: the `host` argument for controllers isn't usually needed, but on puma,
# the default host IP isn't the correct one to use for workers to communicate
# with the main process.
if (isTRUE(hpc))  {
  host_ip <- nanonext::ip_addr()["enp1s0f1np1"]
} else {
  host_ip <- nanonext::ip_addr()[1]
}
